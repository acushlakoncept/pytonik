...
Find and uncomment #  if only is comment with # ``LoadModule rewrite_module modules/mod_rewrite.so``and
...
open the httpd.conf file``sudo nana /etc/httpd/conf/httpd.conf`` now set for editing.
